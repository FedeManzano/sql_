...
UPDATE Ejemplar SET Estado = 1

-- Esta en Lista negra
SELECT dbo.f_Esta_Lista_Negra('C-107')

-- Clientes en estado NO OK
SELECT * FROM vw_Clientes_NO_OK

-- Clientes en estado OK
SELECT * FROM vw_Clientes_OK

-- Precio de prestamo por ejemplar cambio
EXEC sp_Actualizar_Precio 1500.35

-- Precio total a abonar por cliente
SELECT dbo.f_Precio_Total_Abonar('A-071') AS ADEUDA

-- Clientes y su estado descriptivo
SELECT * FROM vw_Clientes_Estado_Descriptivo

SELECT *
FROM vw_Clientes_Estado_Descriptivo CED 
WHERE CED.CODIGO IN 
(
    SELECT CodSocio FROM dbo.f_Lista_Negra()
) 

-- DELETE FROM Cliente WHERE CodSocio = 'A-071'
UPDATE Cliente SET Estado = 1 WHERE CodSocio = 'A-071'

-- 
SELECT * FROM dbo.f_Ejemplares_Disponibles_Pelicula(10)
SELECT * FROM dbo.f_Disponibilidad_De_Pelicula(10)

SELECT * FROM vw_Estado_Prestamos
